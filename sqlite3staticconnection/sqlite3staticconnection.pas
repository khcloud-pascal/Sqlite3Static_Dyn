{
    ...
    against a static sqlite3 library.  It is essentially just a copy of
    sqlite3conn.pp that uses my reworked sqlite3staticlib rather than
    sqlite3dyn.
}
...
uses
  classes, db, bufdataset, sqldb, types, LResources, sqlite3staticlib;
...
type
  ...
  TSQLite3StaticConnection = class(TSQLConnection)
  protected
    fhandle: psqlite3;
  ...
  { TSQLite3StaticConnectionDef }

  TSQLite3StaticConnectionDef = class(TConnectionDef)
...
type
 ...
 TSQLite3Cursor = class(tsqlcursor)
  private
   fhandle : psqlite3;
   fconnection: TSQLite3StaticConnection;
...
constructor TSQLite3StaticConnection.Create(AOwner: TComponent);
begin
...
procedure TSQLite3StaticConnection.LoadBlobIntoBuffer(FieldDef: TFieldDef; ABlobBuf: PBufBlobField; cursor: TSQLCursor; ATransaction : TSQLTransaction);
...
function TSQLite3StaticConnection.AllocateTransactionHandle: TSQLHandle;
begin
 result:= tsqlhandle.create;
end;

function TSQLite3StaticConnection.AllocateCursorHandle: TSQLCursor;
...
procedure TSQLite3StaticConnection.DeAllocateCursorHandle(var cursor: TSQLCursor);
begin
  freeandnil(cursor);
end;

function TSQLite3StaticConnection.StrToStatementType(s: string): TStatementType;
begin
  S:=Lowercase(s);
...
procedure TSQLite3StaticConnection.PrepareStatement(cursor: TSQLCursor;
...
procedure TSQLite3StaticConnection.UnPrepareStatement(cursor: TSQLCursor);
...
procedure TSQLite3StaticConnection.AddFieldDefs(cursor: TSQLCursor; FieldDefs: TFieldDefs);
var
 i, fi : integer;
...
procedure TSQLite3StaticConnection.Execute(cursor: TSQLCursor;
...
function TSQLite3StaticConnection.LoadField(cursor : TSQLCursor; FieldDef : TFieldDef; buffer : pointer; out CreateBlob : boolean) : boolean;
...
function TSQLite3StaticConnection.Fetch(cursor: TSQLCursor): boolean;

begin
  Result:=TSQLite3Cursor(cursor).Fetch;
end;

procedure TSQLite3StaticConnection.FreeFldBuffers(cursor: TSQLCursor);
begin
 //dummy
end;

function TSQLite3StaticConnection.GetTransactionHandle(trans: TSQLHandle): pointer;
begin
 result:= nil;
end;

function TSQLite3StaticConnection.Commit(trans: TSQLHandle): boolean;
begin
  execsql('COMMIT');
  result:= true;
end;

function TSQLite3StaticConnection.RollBack(trans: TSQLHandle): boolean;
begin
  execsql('ROLLBACK');
  result:= true;
end;

function TSQLite3StaticConnection.StartDBTransaction(trans: TSQLHandle; aParams: string): boolean;
begin
  execsql('BEGIN');
  result:= true;
end;

procedure TSQLite3StaticConnection.CommitRetaining(trans: TSQLHandle);
begin
  commit(trans);
  execsql('BEGIN');
end;

procedure TSQLite3StaticConnection.RollBackRetaining(trans: TSQLHandle);
begin
  rollback(trans);
  execsql('BEGIN');
end;

procedure TSQLite3StaticConnection.DoInternalConnect;
var
  filename: ansistring;
begin
  Inherited;
...
procedure TSQLite3StaticConnection.DoInternalDisconnect;
...
function TSQLite3StaticConnection.GetHandle: pointer;
begin
  result:= fhandle;
end;

procedure TSQLite3StaticConnection.checkerror(const aerror: integer);
...
procedure TSQLite3StaticConnection.execsql(const asql: string);
var
 err  : pchar;
 str1 : string;
 res  : integer;
...
function TSQLite3StaticConnection.stringsquery(const asql: string): TArrayStringArray;
begin
  SetLength(result,0);
  checkerror(sqlite3_exec(fhandle,pchar(asql),@execscallback,@result,nil));
end;

function TSQLite3StaticConnection.GetSchemaInfoSQL(SchemaType: TSchemaType;
...
procedure TSQLite3StaticConnection.UpdateIndexDefs(IndexDefs: TIndexDefs; TableName: string);
var
...
function TSQLite3StaticConnection.RowsAffected(cursor: TSQLCursor): TRowsCount;
begin
...
function TSQLite3StaticConnection.RefreshLastInsertID(Query: TCustomSQLQuery; Field: TField): Boolean;
begin
  Field.AsLargeInt:=GetInsertID;
  Result:=True;
end;

function TSQLite3StaticConnection.GetInsertID: int64;
begin
 result:= sqlite3_last_insert_rowid(fhandle);
end;

procedure TSQLite3StaticConnection.GetFieldNames(const TableName: string;
  List: TStrings);
begin
  GetDBInfo(stColumns,TableName,'name',List);
end;

function TSQLite3StaticConnection.GetConnectionInfo(InfoType: TConnInfoType): string;
begin
  Result:='';
  try
    //InitializeSQLite;
    case InfoType of
      citServerType:
        Result:=TSQLite3StaticConnectionDef.TypeName;
      citServerVersion,
      citClientVersion:
      ...
      citClientName:
        Result:=TSQLite3StaticConnectionDef.LoadedLibraryName;
    else
...
procedure TSQLite3StaticConnection.CreateDB;
...
procedure TSQLite3StaticConnection.DropDB;
begin
  CheckDisConnected;
...
procedure TSQLite3StaticConnection.CreateCollation(const CollationName: string;
...
procedure TSQLite3StaticConnection.LoadExtension(LibraryFile: string);
var
  LoadResult: integer;
begin
...
class function TSQLite3StaticConnectionDef.TypeName: string;
begin
  Result := 'SQLite3';
end;

class function TSQLite3StaticConnectionDef.ConnectionClass: TSQLConnectionClass;
begin
  Result := TSQLite3StaticConnection;
end;

class function TSQLite3StaticConnectionDef.Description: string;
begin
  Result := 'Connect to a SQLite3 database directly';
end;

procedure Register;
begin
  {$I sqlite3staticconnection_icon.lrs}
  RegisterComponents('SQLdb',[TSQLite3StaticConnection]);
end;


initialization
  RegisterConnection(TSQLite3StaticConnectionDef);

finalization
  UnRegisterConnection(TSQLite3StaticConnectionDef);
